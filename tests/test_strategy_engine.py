...
def test_a_v2_verified_suspension_precedes_missing_cb_close_price(tmp_path):
    """Match 35914 on 2026-09-11 without using a synthetic parent price."""
    with connect(tmp_path / "strategy.db") as connection:
        _seed_a_v1_data(connection)
        connection.execute(
            "UPDATE cb_daily SET cb_code='35914', cb_name='艾笛森四' WHERE cb_code='12345'"
        )
        connection.execute(
            """UPDATE cb_daily SET close_price=NULL, volume_lots=0, reference_price=107
               WHERE cb_code='35914' AND trade_date='2026-08-12'"""
        )
        connection.execute(
            """INSERT INTO cb_master (
                cb_code, cb_name, stock_code, stock_name, issue_date, maturity_date,
        ...
        connection.execute(
            """INSERT INTO conversion_price_events
                (cb_code, effective_date, conversion_price, source, source_url, collected_at)
                VALUES ('35914', '2024-11-22', 24.5, 'test', 'https://example.test', 'x')"""
        )
        connection.execute("DELETE FROM stock_daily_market")
        upsert_stock_daily_coverage(connection, [{
            "trade_date": "2026-08-12", "stock_code": "3591", "market": "TWSE",
            ...
            "availability_evidence_json": json.dumps({
                "last_trade_date": "2026-08-09", "recovery_date": "2026-08-21",
            }),
            "checked_at": "2026-08-12T00:00:00+00:00",
        }])
        ...
        assert result["values"]["parent_stock_availability"] == "VERIFIED_SUSPENDED"
        assert connection.execute(
            "SELECT reference_price FROM cb_daily WHERE cb_code='35914' AND trade_date='2026-08-12'"
        ).fetchone()[0] == 107
        assert run_a_v2(connection, ["2026-08-12"])["signals_inserted"] == 0


def test_a_v2_null_cb_close_without_verified_suspension_keeps_close_reason(tmp_path):
    with connect(tmp_path / "strategy.db") as connection:
        _seed_a_v1_data(connection)
        connection.execute(
            "UPDATE cb_daily SET close_price=NULL WHERE cb_code='12345' AND trade_date='2026-08-12'"
        )
        upsert_stock_daily_coverage(connection, [{
            "trade_date": "2026-08-12", "stock_code": "1234", "market": "TWSE",
            "status": "COMPLETE", "reason": None,
            "source_url": "https://example.test", "response_date": "2026-08-12",
