...
版本為 `v1`，不讀取、更不覆寫 A-v2、B-v1 或 G-v1 記錄。
...
不會覆蓋既有 C-v1、A-v2、B-v1 或 G-v1 訊號。

## 每日整合與 Dashboard

每日 workflow 在母股日行情完成後，先發布 A-v2，再依序執行 B-v1、C-v1、G-v1。C 使用 `continue-on-error`，因此 C 的錯誤不會阻斷 G、Dashboard 或 snapshot 發布；A-v2 是發布前置，若它失敗則主流程會停止。Dashboard 讀取已保存快照，不重新計算策略。
首頁以 A／B／C／G 標籤顯示訊號；`strategy-c.html` 可依日期檢視各區前二名、完整條件與資料不足原因，
